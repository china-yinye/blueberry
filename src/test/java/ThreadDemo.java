...
public class ThreadDemo {
    ...
    final static String startTime = format.format(new Date());
    ...
    public static void main(String[] args) throws InterruptedException{
        ...
        executor.execute(new Stats("Task-A", 1000,latch));
        executor.execute(new Stats("Task-B", 2000,latch));
        executor.execute(new Stats("Task-C", 3000,latch));
        executor.execute(new Stats("Task-D", 4000,latch));
        executor.execute(new Stats("Task-E", 5000,latch));
        executor.execute(new Stats("Task-F", 6000,latch));
        executor.execute(new Stats("Task-G", 7000,latch));
        executor.execute(new Stats("Task-H", 8000,latch));
        // 等待所有人任务结束
        latch.await();
        System.out.println("所有的统计任务执行完成:" + format.format(new Date()));
    }
    ...
    static class Stats implements Runnable {
        ...
        public void run() {
            try{
                ...
                System.out.println(statsName + " do stats complete at "+ format.format(new Date()));
                //单次任务结束，计数器减一
                latch.countDown();
